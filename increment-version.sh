...
cat deployment.yaml | sed -re "s/image:[[:space:]]robinhoodis\/ffmpeg:.*/image: robinhoodis\/ffmpeg:${CONTAINERVERSION}/" > deployment.yaml.tmp && mv deployment.yaml.tmp deployment.yaml
